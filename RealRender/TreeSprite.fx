...
SamplerState samTriLinearClamp : register(s0);
...
float4 PS(GeoOut pin) : SV_Target
{
	...
	float3 toEye = gPEyePosW - pin.PosW;
	...
	bool useTexure = true;	//임시
	if (useTexure)
	{
		// Sample texture.
		float3 uvw = float3(pin.Tex, pin.PrimID%4);
		texColor = gTreeMapArray.Sample(samTriLinearClamp, uvw);

		bool alphaClip = true;	//임시
		if( alphaClip)
		{
