...
bool D3D::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	result = m_pDevice->CreateRenderTargetView(pBackBufferPtr, NULL, &m_pRrenderTargetView);
	if(FAILED(result))
	{
		return false;
	}
	...
	m_pDeviceContext->OMSetRenderTargets(1, &m_pRrenderTargetView, m_pDepthStencilView);
...
void D3D::Shutdown()
{
	...
	if(m_pRrenderTargetView)
	{
		m_pRrenderTargetView->Release();
		m_pRrenderTargetView = nullptr;
	}
...
void D3D::BeginScene(float red, float green, float blue, float alpha)
{
	...
	m_pDeviceContext->ClearRenderTargetView(m_pRrenderTargetView, color);
